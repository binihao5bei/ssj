...
headers = {
    'Host' : 'm.weibo.cn',
    'Referer' : 'https://m.weibo.cn/u/1980923093',
...
# 获取每一页的请求数据返回的是json格式
    ...
    def get_data(self):
        print("开始爬取【麒麟软件有限公司】的内容！")
        print("本次爬取今日： [" +self.nowdate + "] 的数据记录。")
        
        datalists=[]
        data=Wb_neokylin.get_page(1).get('data').get('cards')
        ...
        for item in items:
            ...
            #print("正在爬取第{}条内容。请稍后......".format(items.index))
                ...
                if "<a" in text:
                    if "<span" not in text:
                        title=('【麒麟软件】'+text.split("<")[0]).replace('&','&amp;')
                    elif "<span" in text:
                        continue 
                elif "<a" not in text:
                    continue       
